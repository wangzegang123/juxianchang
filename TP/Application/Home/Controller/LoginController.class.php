...
//                    $model=M("member");
                    ...
                    $id=$Model->query("select member_id,member_name from jxc_member where member_name='{$name}' and member_pwd='{$pwd}'");
                    if($arr){
                        session("mid",$id[0]['member_id']);
                        session("member_name",$id[0]['member_name']);
                        $this->redirect("Index/activity_person");
                    }else{
                        echo "<script>alert('用户名或' +
 '密码有误');location='login'</script>";
                    }
                } else {
                    echo '{"status":"fail"}';
                }
